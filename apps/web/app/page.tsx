...
import { ancestries, archetypes as compactArchetypes, classes, feats, optionGroups, skills, traits } from "./character-catalogue";
...
import { EquipmentPanel, equipmentCombatBonuses, equippedWeaponAttacks, type CoinPurse, type InventoryEntry } from "./equipment-panel";
...
const prerequisiteFeatureKey = (value: string) => value.toLowerCase().replace(/[’']/g, "").replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
const archetypeIdsByClass = Object.fromEntries(classes.map((characterClass) => [characterClass.id, compactArchetypes.filter((archetype) => archetype.classId === characterClass.id).map((archetype) => archetype.id)]));
...
export default function Home() {
  ...
  const [archetypeStacksByClass, setArchetypeStacksByClass] = useState<Record<string, string[]>>({});
  const [prestigeSpellcastingTargets, setPrestigeSpellcastingTargets] = useState<Record<string, string[]>>({});
  const [ancestryId, setAncestryId] = useState("human");
  const [selectedAlternateRacialTraitIds, setSelectedAlternateRacialTraitIds] = useState<string[]>([]);
  ...
  const [favoredClassAlternateBonuses, setFavoredClassAlternateBonuses] = useState<Record<string, number>>({});
  ...
  const { spells, loading: spellCatalogueLoading } = useSpellCatalogue(activeTab === "spells");
  const hasSelectedArchetype = Boolean(archetypeId)
    || Object.values(additionalArchetypeIds).some(Boolean)
    || Object.values(archetypeStacksByClass).some((ids) => ids.length > 0);
  const { archetypes, loading: archetypeCatalogueLoading } = useArchetypeCatalogue(hasSelectedArchetype);
  ...
  const availableArchetypes = useMemo(() => archetypes.filter((item) => item.classId === baseCharacterClass.id), [baseCharacterClass.id]);
  const primaryArchetypeIds = archetypeStacksByClass[classId] ?? (archetypeId ? [archetypeId] : []);
  const selectedArchetypes = useMemo(() => primaryArchetypeIds.flatMap(id => availableArchetypes.find(item => item.id === id) ?? []), [availableArchetypes, primaryArchetypeIds.join("|")]);
  const selectedArchetypeReplacesText = selectedArchetypes.map((item) => item.replacesText).filter(Boolean).join("; ");
  const characterClass = useMemo(() => applyArchetypes(baseCharacterClass, selectedArchetypes), [baseCharacterClass, selectedArchetypes]);
  const additionalCharacterClasses = useMemo(() => additionalClassLevels.map((entry) => {
    ...
    const selectedIds = archetypeStacksByClass[entry.classId] ?? (additionalArchetypeIds[entry.classId] ? [additionalArchetypeIds[entry.classId]] : []);
    const selectedAdditionalArchetypes = selectedIds.flatMap(id => archetypes.find(item => item.id === id && item.classId === entry.classId) ?? []);
    return baseClass ? applyArchetypes(baseClass, selectedAdditionalArchetypes) : undefined;
  }).filter((item): item is typeof classes[number] => Boolean(item)), [additionalArchetypeIds, additionalClassLevels, archetypeStacksByClass]);
  ...
  const secondarySpellcastingLevel = effectiveSpellcastingLevelMap[secondaryClassId] ?? secondaryClassLevel;
  const ancestry = ancestries.find((item) => item.id === ancestryId) ?? ancestries[0];
  const selectedAlternateRacialTraits = (ancestry.alternateTraits ?? []).filter((trait) => selectedAlternateRacialTraitIds.includes(trait.id));
  const replacedRacialTraitIds = new Set(selectedAlternateRacialTraits.flatMap((trait) => trait.replaces));
  const effectiveRacialTraits = ancestry.traits.filter((trait) => !replacedRacialTraitIds.has(trait.id));
  ...
  }, [abilities.intelligence, additionalCharacterClasses.length, ancestry, ancestryBonusFeats, characterClass, classLevels, favoredClassSkillRanks, level, progressionClasses, selectedAlternateRacialTraitIds]);
  const unresolvedChoiceCount = progression.features.filter(feature =>
    feature.level <= level && feature.choiceRequired && !selectedOptions[feature.id]
  ).length;
  ...
  const featSpellLevels = useMemo(() => progressionClasses.reduce<Partial<Record<"prepared" | "spontaneous", number>>>((levels, entry) => {
    const classLevel = effectiveSpellcastingLevelMap[entry.id] ?? classLevelMap[entry.id] ?? 0;
    if (!entry.spellcasting || classLevel < 1) return levels;
    try {
      const casting = spellcastingProgression(entry, classLevel, { abilityScore: abilities[entry.spellcasting.ability] });
      if (casting) levels[entry.spellcasting.castingType] = Math.max(levels[entry.spellcasting.castingType] ?? 0, casting.maximumSpellLevel);
    } catch {
  ...
  const featContext = useMemo(() => ({ classId: characterClass.id, classLevels: classLevelMap, ancestryId, size: ancestry.size, classLevel: level, casterLevel, spellLevels: featSpellLevels, abilities, baseAttackBonus: progression.baseAttackBonus, saves: progression.saves, skillRanks, featureIds: [...new Set([...progression.features.flatMap((feature) => { const key = prerequisiteFeatureKey(feature.name); return [feature.id, key, key.replace(/-\d+$/, ""), ...(feature.progressionKey ? [feature.progressionKey] : [])]; }), ...selectedOptionFeatureIds])], spellIds: featSpellIds, selectedIds: selectedClassFeatIds, selectedFeatChoices }), [abilities, ancestry.size, ancestryId, casterLevel, characterClass.id, classLevelMap, featSpellIds, featSpellLevels, level, progression.baseAttackBonus, progression.features, progression.saves, selectedClassFeatIds, selectedFeatChoices, selectedOptionFeatureIds, skillRanks]);
  ...
  const classOptionChoices = choiceFeatures.map((feature) => {
    ...
    const options = feature.requiredOptionId && !selectedIds.includes(feature.requiredOptionId) ? [] : baseOptions;
    return { id: feature.id, name: feature.name, level: feature.level, classLevel: featureClassLevel, options, selected: options.find((option) => option.id === selectedOptions[feature.id]), requiredOptionId: feature.requiredOptionId, requiredOptionMessage: feature.requiredOptionMessage };
  });
  ...
  const preparedCasting = useMemo(() => characterClass.spellcasting && !isSpontaneous ? spellcastingProgression(characterClass, primarySpellcastingLevel, { abilityScore: castingAbilityScore }) : null, [castingAbilityScore, characterClass, isSpontaneous, primarySpellcastingLevel]);
  const spontaneousCasting = useMemo(() => isSpontaneous ? spontaneousSpellcastingProgression(characterClass, primarySpellcastingLevel, { abilityScore: castingAbilityScore }) : null, [castingAbilityScore, characterClass, isSpontaneous, primarySpellcastingLevel]);
  ...
  const baseAvailableSpells = useMemo(() => hasSpellcasting ? spellsAvailableToClass(spells, characterClass.id, maximumSpellLevel, characterClass.spellListAdditions) : [], [characterClass.id, characterClass.spellListAdditions, hasSpellcasting, maximumSpellLevel, spells]);
  const bloodlineSpells = useMemo(() => classId === "sorcerer"
    ...
    : [], [characterClass.id, classId, maximumSpellLevel, primaryClassLevel, selectedBloodline, spells]);
  const mysterySpells = useMemo(() => classId === "oracle"
    ...
    : [], [characterClass.id, classId, maximumSpellLevel, primaryClassLevel, selectedMystery, spells]);
  ...
  const secondaryPreparedCasting = useMemo(() => secondaryCharacterClass?.spellcasting && !secondaryIsSpontaneous ? spellcastingProgression(secondaryCharacterClass, secondarySpellcastingLevel, { abilityScore: secondaryCastingAbilityScore }) : null, [secondaryCastingAbilityScore, secondaryCharacterClass, secondaryIsSpontaneous, secondarySpellcastingLevel]);
  const secondarySpontaneousCasting = useMemo(() => secondaryCharacterClass && secondaryIsSpontaneous ? spontaneousSpellcastingProgression(secondaryCharacterClass, secondarySpellcastingLevel, { abilityScore: secondaryCastingAbilityScore }) : null, [secondaryCastingAbilityScore, secondaryCharacterClass, secondaryIsSpontaneous, secondarySpellcastingLevel]);
  ...
  const secondaryBaseSpells = useMemo(() => secondaryHasSpellcasting && secondaryCharacterClass ? spellsAvailableToClass(spells, secondaryCharacterClass.id, secondaryMaximumSpellLevel, secondaryCharacterClass.spellListAdditions) : [], [secondaryCharacterClass, secondaryHasSpellcasting, secondaryMaximumSpellLevel, spells]);
  const secondaryBloodlineSpells = useMemo(() => secondaryCharacterClass?.id === "sorcerer"
    ...
    : [], [secondaryCharacterClass, secondaryClassLevel, secondaryMaximumSpellLevel, secondarySelectedBloodline, spells]);
  const secondaryMysterySpells = useMemo(() => secondaryCharacterClass?.id === "oracle"
    ...
    : [], [secondaryCharacterClass, secondaryClassLevel, secondaryMaximumSpellLevel, secondarySelectedMystery, spells]);
  ...
  }), [secondaryCharacterClass, secondaryMaximumSpellLevel, selectedOptionSpellChoices, spells]);
  ...
  const alternateBardicPerformanceRounds = alternateFavoredClassRewards
    ...
    .reduce((total, reward) => total + alternateRewardValue(reward, favoredClassAlternateBonuses[reward.id] ?? 0), 0);
  const bardicPerformanceMaximum = bardClassLevel > 0 ? bardicPerformanceRounds(bardClassLevel, combat.abilityModifiers.charisma) + alternateBardicPerformanceRounds : 0;
  ...
  const savedArchetypeIdsByClass = Object.fromEntries(classLevels.flatMap((entry, index) => {
    const selectedId = (archetypeStacksByClass[entry.classId] ?? [index === 0 ? archetypeId : additionalArchetypeIds[entry.classId]]).filter(Boolean)[0];
    return selectedId ? [[entry.classId, selectedId]] : [];
  }));
  const characterDraft: CharacterDraftV1 = { version: 1, name, classId, classLevels, archetypeId, archetypeIdsByClass: savedArchetypeIdsByClass, archetypeStacksByClass, prestigeSpellcastingTargets, ancestryId, selectedAlternateRacialTraitIds, level, humanAbility, baseAbilities, pointBuyBudget, abilityBoosts, favoredClassHitPoints, favoredClassSkillRanks, favoredClassAlternateBonuses, selectedFeatIds, selectedTraitIds, selectedTraitChoices, selectedFeatChoices, skillRanks, selectedOptions, preparedSpells: selectedSpellIds, preparedSpellsByClass, spellSlotUses: Object.fromEntries(Object.entries(spellSlotUses)), spellSlotUsesByClass, arcaneReservoir: classLevelMap.arcanist ? reservoirPoints : null, bardicPerformanceUsed: bardClassLevel > 0 ? bardicPerformanceUsed : 0, wildShapeUsed: druidClassLevel > 0 ? wildShapeUsed : 0, currentHitPoints, temporaryHitPoints, activeEffects, inventory, coins };
  const combatAttacks = equippedWeaponAttacks(inventory, progression.baseAttackBonus, combat.abilityModifiers.strength, combat.abilityModifiers.dexterity, selectedFeatBonuses.weaponBonuses);
  ...
  useEffect(() => {
    try {
      ...
      if (library.characters.length === 0 && legacy) {
        const draft = normalizeCharacterDraft(JSON.parse(legacy), { classIds: classes.map((item) => item.id), ancestryIds: ancestries.map((item) => item.id), archetypeIds: archetypes.map((item) => item.id), archetypeIdsByClass });
        if (draft) {
  ...
  const applyCharacterDraft = (value: unknown, successNotice: string) => {
    ...
    const draftArchetypes = (selectedClassId: string) => (draft.archetypeStacksByClass?.[selectedClassId] ?? (draft.archetypeIdsByClass[selectedClassId] ? [draft.archetypeIdsByClass[selectedClassId]] : []))
      .flatMap((id) => archetypes.find((item) => item.id === id && item.classId === selectedClassId) ?? []);
    const draftBaseClass = classes.find((item) => item.id === draft.classId) ?? classes[0];
    const draftClass = applyArchetypes(draftBaseClass, draftArchetypes(draft.classId));
    ...
    const draftSecondaryClass = draftSecondaryBaseClass ? applyArchetypes(draftSecondaryBaseClass, draftArchetypes(draftSecondaryBaseClass.id)) : undefined;
    const draftProgressionClasses = draft.classLevels.flatMap((entry) => {
      const baseClass = classes.find((item) => item.id === entry.classId);
      return baseClass ? [applyArchetypes(baseClass, draftArchetypes(entry.classId))] : [];
    });
    const draftEffectiveSpellcastingLevels = effectiveSpellcastingLevels(draftProgressionClasses, draft.classLevels, draft.prestigeSpellcastingTargets);
    const draftPrimarySpellcastingLevel = draftEffectiveSpellcastingLevels[draft.classId] ?? draftPrimaryLevel;
    const draftSecondarySpellcastingLevel = draftSecondaryLevel ? draftEffectiveSpellcastingLevels[draftSecondaryLevel.classId] ?? draftSecondaryLevel.level : 1;
    ...
    const draftPreparedCasting = draftClass.spellcasting && !draftIsSpontaneous ? spellcastingProgression(draftClass, draftPrimarySpellcastingLevel, { abilityScore: draftAbilityScore }) : null;
    const draftSpontaneousCasting = draftIsSpontaneous ? spontaneousSpellcastingProgression(draftClass, draftPrimarySpellcastingLevel, { abilityScore: draftAbilityScore }) : null;
    const draftCasting = draftSpontaneousCasting ?? draftPreparedCasting;
    const draftBaseSpells = draftCasting ? spellsAvailableToClass(spells, draftClass.id, draftCasting.maximumSpellLevel, draftClass.spellListAdditions) : [];
    ...
    const draftSecondaryPrepared = draftSecondaryClass?.spellcasting && !draftSecondaryIsSpontaneous ? spellcastingProgression(draftSecondaryClass, draftSecondarySpellcastingLevel, { abilityScore: draftSecondaryAbilityScore }) : null;
    const draftSecondarySpontaneous = draftSecondaryClass && draftSecondaryIsSpontaneous ? spontaneousSpellcastingProgression(draftSecondaryClass, draftSecondarySpellcastingLevel, { abilityScore: draftSecondaryAbilityScore }) : null;
    const draftSecondaryCasting = draftSecondarySpontaneous ?? draftSecondaryPrepared;
    const draftSecondaryBaseSpells = draftSecondaryClass && draftSecondaryCasting ? spellsAvailableToClass(spells, draftSecondaryClass.id, draftSecondaryCasting.maximumSpellLevel, draftSecondaryClass.spellListAdditions) : [];
    ...
    const normalizedDraftSecondarySpells = !draftSecondaryClass ? [] : draftSecondaryIsSpontaneous
      ? normalizeKnownSpells(draftSecondarySelections, draftSecondarySpells, draftSecondaryClass.id, draftSecondarySpontaneous?.known ?? [], draftAllSecondaryGranted.map((spell) => spell.id))
  ...
  return <main id="character-builder-main" tabIndex={-1}>
    ...
    {unresolvedChoiceCount > 0 && <button type="button" className="unresolved-choice-banner" onClick={() => { setActiveTab("features"); setSidebarOpen(false); }}>
      <strong>{unresolvedChoiceCount} choice{unresolvedChoiceCount === 1 ? "" : "s"} remaining</strong>
    ...
    <section id="character-tab-panel" className="tab-panel" role="tabpanel" aria-labelledby={`character-tab-${activeTab}`} tabIndex={0}>
      ...
      {activeTab === "actions" && <div className="actions-workspace"><CombatPanel combat={combat} modifierSources={selectedFeatBonuses.sources} conditionalModifiers={selectedTraitBonuses.conditionalModifiers} /><ActivePlayPanel maximumHitPoints={combat.averageHitPoints} currentHitPoints={currentHitPoints ?? combat.averageHitPoints} temporaryHitPoints={temporaryHitPoints} attacks={combatAttacks} effects={activeEffects} onCurrentHitPointsChange={setCurrentHitPoints} onTemporaryHitPointsChange={setTemporaryHitPoints} onEffectsChange={setActiveEffects} /></div>}
      ...
      {activeTab === "features" && <div className="feature-workspace">{archetypeCatalogueLoading && <p className="catalogue-loading" role="status">Loading complete archetype rules…</p>}<ClassFeatures level={level} className={additionalClassLevels.length > 0 ? classLevels.map((entry) => `${classes.find((item) => item.id === entry.classId)?.name ?? entry.classId} ${entry.level}`).join(" / ") : characterClass.name} features={progression.features} dailyResources={classDailyResources} />{classOptionChoices.length > 0 && <ClassOptions choices={classOptionChoices} selectedOptions={selectedOptions} classLevel={primaryClassLevel} charismaModifier={combat.abilityModifiers.charisma} archetypeReplacesText={selectedArchetypeReplacesText || undefined} onOptionChange={updateClassOption} />}</div>}
