...
export default function Home() {
  ...
  const featContext = useMemo(() => ({ classId: characterClass.id, classLevel: level, casterLevel: "spellcasting" in characterClass ? level : 0, abilities, baseAttackBonus: progression.baseAttackBonus, skillRanks, featureIds: progression.features.map((feature) => feature.id) }), [abilities, characterClass, level, progression.baseAttackBonus, progression.features, skillRanks]);
  const featChoices = featSlots.map((slot) => { const selected = feats.find((feat) => feat.id === selectedFeatIds[slot.index]); const otherFeatIds = selectedFeatIds.filter((_, index) => index !== slot.index); const context = { ...featContext, selectedIds: otherFeatIds }; const checks = selected ? featPrerequisiteResults(selected, context) : []; return { ...slot, selected, checks, eligibleFeatIds: feats.filter((feat) => prerequisitesMet(feat.prerequisites, context)).map((feat) => feat.id) }; });
