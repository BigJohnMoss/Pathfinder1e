...
export default function Home() {
  ...
  const updatePreparedSpells = (spellIds: string[]) => setPreparedSpells(normalizePreparedSpellsWithOpposition(spellIds, availableSpells, characterClass.id, preparedLimits, oppositionSchoolIds));
  useEffect(() => setPreparedSpells((current) => { const next = normalizePreparedSpellsWithOpposition(current, availableSpells, characterClass.id, preparedLimits, oppositionSchoolIds); return next.length === current.length && next.every((id, index) => id === current[index]) ? current : next; }), [availableSpells, characterClass.id, oppositionSchoolIds, preparedLimits]);
  ...
  return <main>
    ...
    <section className="tab-panel" aria-live="polite">
      ...
      {activeTab === "spells" && (spellcasting ? <Spellbook spells={availableSpells} classId={characterClass.id} className={characterClass.name} castingAbilityName={castingAbility ? labels[castingAbility] : "casting ability"} slots={spellcasting.slots} preparedLimits={preparedLimits} spellDcs={spellDcs} maximumSpellLevel={maximumSpellLevel} preparedSpellIds={preparedSpells} onPreparedSpellIdsChange={updatePreparedSpells} slotUses={spellSlotUses} onSlotUsesChange={updateSpellSlotUses} reservoir={reservoir ? { current: reservoirPoints, ...reservoir } : null} onReservoirChange={updateReservoir} onRefreshDay={refreshDay} oppositionSchoolIds={oppositionSchoolIds} /> : <p className="empty-tab">This class does not cast spells.</p>)}
