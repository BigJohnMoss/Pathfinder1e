...
export default function Home() {
  ...
  const savedArchetypeIdsByClass = Object.fromEntries(classLevels.flatMap((entry, index) => {
    const selectedId = index === 0 ? archetypeId : additionalArchetypeIds[entry.classId];
  ...
  const applyCharacterDraft = (value: unknown, successNotice: string) => {
    ...
    const draftProgressionClasses = draft.classLevels.flatMap((entry) => {
      const baseClass = classes.find((item) => item.id === entry.classId);
      return baseClass ? [applyArchetype(baseClass, archetypes.find((item) => item.id === draft.archetypeIdsByClass[entry.classId] && item.classId === entry.classId))] : [];
    });
