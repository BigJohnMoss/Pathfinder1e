...
export default function Home() {
  ...
  const normalizeSelectedSpells = (spellIds: string[]) => isSpontaneous
    ? normalizeKnownSpells(spellIds, availableSpells, characterClass.id, knownLimits)
  ...
  useEffect(() => setSelectedSpellIds((current) => { const next = normalizeSelectedSpells(current); return next.length === current.length && next.every((id, index) => id === current[index]) ? current : next; }), [availableSpells, characterClass.id, isSpontaneous, knownLimits, oppositionSchoolIds, preparedLimits]);
