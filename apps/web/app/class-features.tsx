  ...
  return <section className="features">
    ...
    <ol>{features.map((feature) => <li key={feature.id}>
      ...
      <div><strong>{feature.name}</strong><p>{feature.summary}</p>{feature.teamworkFeatSharing && <div className="passive-feat-sharing" role="region" aria-label={`${feature.name} shared teamwork feats`}><strong>Shared with {feature.teamworkFeatSharing.targetLabel}</strong>{selectedFeats.filter((feat) => feat.type === feature.teamworkFeatSharing?.featType).length ? <div className="passive-feat-chips">{selectedFeats.filter((feat) => feat.type === feature.teamworkFeatSharing?.featType).map((feat) => <span key={feat.id}>{feat.name}</span>)}</div> : <p>No teamwork feats selected yet.</p>}<small>{feature.teamworkFeatSharing.summary}{feature.teamworkFeatSharing.ignorePrerequisites ? ` The ${feature.teamworkFeatSharing.targetLabel.toLowerCase()} does not need to meet their prerequisites.` : ""}</small></div>}{feature.progressionProfiles?.filter((profile) => !profile.requiredOptionId || selectedOptionSet.has(profile.requiredOptionId)).map((profile) => {
        ...
        const multiplier = calculation.inputMultiplierByLevel?.filter((step) => step.level <= calculationLevel).sort((left, right) => left.level - right.level).at(-1)?.multiplier ?? 1;
        ...
        const variableCostRemaining = !action.resourceId || variableCostResource?.maximum === null ? Number.POSITIVE_INFINITY : Math.max(0, (variableCostResource?.maximum ?? 0) - (variableCostResource?.used ?? 0));
        const variableCostMultiplier = action.variableCost?.multiplier ?? 1;
        const variableCostLevelMaximum = action.variableCost?.maximumByLevel?.filter((step) => step.level <= actionLevel).sort((left, right) => left.level - right.level).at(-1)?.maximum;
        const variableCostMaximum = action.variableCost
          ? Math.max(action.variableCost.minimum, Math.min(action.variableCost.maximum ?? Number.POSITIVE_INFINITY, variableCostLevelMaximum ?? Number.POSITIVE_INFINITY, action.variableCost.maximumLevelDivisor ? Math.floor(actionLevel / action.variableCost.maximumLevelDivisor) : Number.POSITIVE_INFINITY, Math.floor(variableCostRemaining / variableCostMultiplier)))
          : 0;
        const enteredVariableCost = variableAmounts[action.id];
        const selectedVariableAmount = action.variableCost ? Math.max(action.variableCost.minimum, Math.min(typeof enteredVariableCost === "number" ? enteredVariableCost : action.variableCost.minimum, variableCostMaximum)) : undefined;
        const variableCost = action.costPerSelectedFeat ? selectedFeatCost : selectedVariableAmount !== undefined ? selectedVariableAmount * variableCostMultiplier : action.cost;
        ...
        const maximumActiveEffects = action.maximumActiveEffects
          ? Math.max(
              action.maximumActiveEffects.minimum ?? 0,
              Math.min(
                action.maximumActiveEffects.maximum ?? Number.POSITIVE_INFINITY,
                (action.maximumActiveEffects.base ?? 0) +
                  (action.maximumActiveEffects.levelDivisor ? Math.floor(actionLevel / action.maximumActiveEffects.levelDivisor) : 0) +
                  (action.maximumActiveEffects.abilityModifier ? abilityModifiers[action.maximumActiveEffects.abilityModifier] ?? 0 : 0),
              ),
            )
          : undefined;
        const activeEffectCount = action.maximumActiveEffects
          ? activeEffects.filter((effect) => effect.name === action.maximumActiveEffects?.name).length
          : 0;
        const activeEffectLimitReached = maximumActiveEffects !== undefined && activeEffectCount >= maximumActiveEffects;
        ...
        const unavailable = unavailableResource || unavailableCost || unavailableMinimumResource || unavailableRecovery || blockedByActorCondition || activeEffectLimitReached || Boolean(action.modes?.length && !availableModes?.length) || (Boolean(action.activeEffect?.selectEquippedWeapon) && !selectedEffectWeaponId);
        ...
        const performanceSaveDcBonus = action.resourceId === "bardicPerformance" ? activeEffects.filter((effect) => effect.target === "performanceSaveDc").reduce((total, effect) => total + effect.bonus, 0) : 0;
        const saveDc = action.savingThrow ? (fixedSaveDc ?? (action.savingThrow.base ?? 0) + Math.floor(saveLevel / (action.savingThrow.levelDivisor ?? 1)) + (action.savingThrow.ability ? abilityModifiers[action.savingThrow.ability] ?? 0 : 0)) + performanceSaveDcBonus : undefined;
        const saveText = action.savingThrow ? `${action.savingThrow.label} DC ${saveDc} negates.` : undefined;
        const effectDescription = [selectedActionFeatNames.length ? action.featSelection?.source === "catalogue" ? `Temporary feat${selectedActionFeatNames.length === 1 ? "" : "s"}: ${selectedActionFeatNames.join(", ")}.` : `Granted teamwork feat${selectedActionFeatNames.length === 1 ? "" : "s"}: ${selectedActionFeatNames.join(", ")}.` : undefined, selectedRecipient ? `Recipients: ${selectedRecipient.label}.` : undefined, effectRange ? `Range: ${effectRange} feet.` : undefined, selectedMode?.summary, effectUpgrade?.description ?? action.activeEffect?.description, saveText].filter(Boolean).join(" ").replaceAll("{amount}", String(selectedVariableAmount ?? 0)).replaceAll("{classLevel}", String(actionLevel)).replaceAll("{doubleClassLevel}", String(actionLevel * 2));
        ...
        const fixedD20Result = action.fixedD20Result?.resultByLevel?.filter((step) => step.level <= actionLevel).sort((left, right) => left.level - right.level).at(-1)?.result ?? action.fixedD20Result?.result;
        ...
        const diceFlatModifier = action.diceRoll?.flatModifierByLevel?.filter((step) => step.level <= actionLevel).sort((left, right) => left.level - right.level).at(-1)?.modifier ?? 0;
        const diceEnteredModifier = action.diceRoll?.modifierInputLabel ? rerollInput.modifier : 0;
        const diceModifier = (action.diceRoll?.abilityModifier ? abilityModifiers[action.diceRoll.abilityModifier] ?? 0 : 0) + diceFlatModifier + diceEnteredModifier;
        const diceResultDivisor = action.diceRoll?.resultDivisorByMode?.find((step) => step.modeId === selectedMode?.id)?.divisor ?? 1;
        const diceModeEffect = action.diceRoll?.modeEffects?.find((effect) => effect.modeId === selectedMode?.id);
        const combatAttackAbility = action.combatRoll?.attack?.abilityModifier ?? (action.combatRoll?.attack?.kind?.startsWith("melee") ? "strength" : "dexterity");
        ...
        const combatAttackCount = action.combatRoll?.attackCountByLevel?.filter((step) => step.level <= actionLevel).sort((left, right) => left.level - right.level).at(-1)?.count ?? 1;
        const combatArmorClassLabel = action.combatRoll?.attack?.kind?.endsWith("touch") ? "touch AC" : "AC";
        ...
        const maximumTargets = action.targetCountByLevel?.filter((step) => step.level <= actionLevel).sort((left, right) => left.level - right.level).at(-1)?.count;
        ...
        const activate = () => {
          const activatedEffectRounds = action.activeEffect?.durationDice ? rollDice(action.activeEffect.durationDice.count, action.activeEffect.durationDice.sides).total : rounds;
          ...
          if (action.activeEffect && onAddEffect) {
            ...
            targets.forEach((target) => onAddEffect({
              ...
              roundsRemaining: activatedEffectRounds,
            ...
            additionalActiveEffects.forEach((effect) => {
              ...
              onAddEffect({ id: `${action.id}-${effect.target}-${Date.now()}-${Math.random()}`, name: effect.name, target: effect.target, bonus, description: effect.description, roundsRemaining: activatedEffectRounds });
            });
            ...
            selectedMode?.activeEffects?.forEach((effect) => {
              const modeBonus = effect.bonusByLevel?.filter((step) => step.level <= actionLevel).sort((left, right) => left.level - right.level).at(-1)?.bonus ?? effect.bonus;
              onAddEffect({ id: `${action.id}-${effect.target}-${Date.now()}-${Math.random()}`, name: effect.label, target: effect.target, bonus: modeBonus, description: effect.description, roundsRemaining: activatedEffectRounds, ...(effect.fastHealing ? { fastHealing: effect.fastHealing } : {}), ...(effect.skillIds?.length ? { skillIds: effect.skillIds } : {}) });
            });
          }
          ...
          if (action.combatRoll && combatDiceCount && combatDieSides) {
            ...
            const attackCount = action.combatRoll.attackCountFromVariableCost ? Math.max(1, selectedVariableAmount ?? 1) : combatAttackCount;
            for (let attackIndex = 0; attackIndex < attackCount; attackIndex += 1) {
              const attackModifier = combatAttackModifier - attackIndex * (action.combatRoll.iterativeAttackPenalty ?? 0);
              const attack = action.combatRoll.attack ? rollD20Check(attackModifier) : undefined;
              const hit = !attack || (attack.natural !== 1 && (attack.natural === 20 || attack.total >= combatInput.touchArmorClass));
              const attackPrefix = attackCount > 1 ? `${action.combatRoll.attackCountFromVariableCost ? "Word" : "Attack"} ${attackIndex + 1}: ` : "";
              if (attack) parts.push(`${attackPrefix}${action.combatRoll.attack!.label}: ${attack.natural}${attackModifier === 0 ? "" : ` ${attackModifier >= 0 ? "+" : "−"} ${Math.abs(attackModifier)}`} = ${attack.total} vs ${combatArmorClassLabel} ${combatInput.touchArmorClass} — ${hit ? "hit" : "miss"}.`);
              if (!hit) continue;
          ...
          if (action.targetEffectRoll && targetEffect && saveDc !== undefined) {
            ...
            if (!succeeded) {
              ...
              const effectDuration = duration.kind === "fixed-rounds" ? duration.rounds : duration.kind === "dice-rounds" ? rollDice(duration.count, duration.sides).total : duration.kind === "level-hours" ? Math.min(999, actionLevel * 600 * (duration.multiplier ?? 1)) : duration.kind === "level-minutes" ? Math.min(999, actionLevel * 10 * (duration.multiplier ?? 1)) : Math.min(999, actionLevel);
              ...
              targetEffect.activeEffects?.forEach((effect, index) => {
                onAddEffect?.({ id: `${action.id}-target-effect-${index}-${Date.now()}-${Math.random()}`, name: `${effect.label} — ${saveEffectTargetName}`, target: effect.target, bonus: effect.bonus, description: effect.description, roundsRemaining: effectDuration });
              });
          ...
          if (action.fixedD20Result && fixedD20Result) setActionResults((current) => ({ ...current, [action.id]: `${action.fixedD20Result!.label}: treat the d20 result as ${fixedD20Result}.` }));
          if (action.diceRoll && diceCount && dieSides) {
            const roll = rollDice(diceCount, dieSides, diceModifier);
            const resolvedTotal = Math.floor(roll.total / diceResultDivisor);
            ...
            if (targetSave && saveDc !== undefined && diceModeEffect?.targetSave) {
              const appliedAmount = saveSucceeded ? diceModeEffect.targetSave.outcome === "half" ? Math.floor(resolvedTotal / 2) : 0 : resolvedTotal;
          ...
          if (action.randomOutcomes?.length) {
          ...
          } else if (action.activeEffect && !action.rerollAction && !action.diceRoll) setActionResults((current) => ({
            ...current,
            [action.id]: action.featSelection
              ...
              : `${effectDescription || action.activeEffect!.name} Active for ${activatedEffectRounds} round${activatedEffectRounds === 1 ? "" : "s"}.`,
          }));
