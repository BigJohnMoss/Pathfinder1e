  ...
  return <section className="features">
    ...
    <ol>{features.map((feature) => <li key={feature.id}>
      <div><strong>{feature.name}</strong><p>{feature.summary}</p>{feature.progressionProfiles?.filter((profile) => !profile.requiredOptionId || selectedOptionSet.has(profile.requiredOptionId)).map((profile) => {
        ...
        const missingRequiredConfirmation = Boolean(action.combatRoll?.confirmations?.some((confirmation) => confirmation.requiredForActivation && !confirmationChecked(confirmation.id)));
        ...
        return <div className="feature-resource-action" key={action.id}>
          ...
          {action.combatRoll?.confirmations?.map((confirmation) => <label key={confirmation.id}><input aria-label={`${action.label} ${confirmation.label}`} type="checkbox" checked={confirmationChecked(confirmation.id)} onChange={(event) => setActionConfirmations((current) => ({ ...current, [`${action.id}:${confirmation.id}`]: event.target.checked }))} />{confirmation.label}</label>)}
