    ...
    <ol>{features.map((feature) => <li key={feature.id}>
      <div><strong>{feature.name}</strong><p>{feature.summary}</p>{feature.progressionProfiles?.filter((profile) => !profile.requiredOptionId || selectedOptionSet.has(profile.requiredOptionId)).map((profile) => {
        ...
        return <div className="feature-resource-action" key={action.id}>
          ...
          <button type="button" disabled={(!action.activeEffect && temporaryHitPoints === undefined && appliedChanges.length === 0 && !action.rerollAction) || unavailable || !targetEligible} title={blockedByActorCondition ? `Unavailable while ${action.actorSavingThrow?.blockedByActiveEffectName}.` : undefined} onClick={activate}>{label}</button>
