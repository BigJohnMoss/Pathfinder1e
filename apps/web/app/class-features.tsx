    ...
    <ol>{features.map((feature) => <li key={feature.id}>
      <div><strong>{feature.name}</strong><p>{feature.summary}</p>{feature.progressionProfiles?.filter((profile) => !profile.requiredOptionId || selectedOptionSet.has(profile.requiredOptionId)).map((profile) => {
        ...
        return <div className="feature-resource-action" key={action.id}>
          ...
          <button type="button" disabled={(!action.activeEffect && temporaryHitPoints === undefined && appliedChanges.length === 0 && !action.rerollAction && !action.spellLikeAbility && !action.diceRoll && !action.combatRoll && !action.targetEffectRoll) || unavailable || !targetEligible || targetHasSaveEffectImmunity} title={blockedByActorCondition ? `Unavailable while ${action.actorSavingThrow?.blockedByActiveEffectName}.` : targetHasSaveEffectImmunity ? `${saveEffectTargetName} is immune to this ability.` : undefined} onClick={activate}>{label}</button>
