...
import type { Alignment, CharacterClass, CharacterClassLevel } from "../../../packages/types/src/index.js";
import type { CharacterArchetype } from "../../../packages/types/src/index.js";
import { characterAlignmentLabel, characterAlignments } from "../../../packages/engine/src/index.js";
import { ArchetypePicker } from "./archetype-picker";

export function CharacterDetails({ name, classId, additionalClassLevels, additionalArchetypeIds, archetypeStacksByClass, prestigeSpellcastingTargets, archetypeId, ancestryId, alignment, level, classes, archetypes, ancestries, saveNotice, autosaveStatus, recoveryAvailable, onRecover, onDismissRecovery, onNameChange, onClassChange, onAdditionalClassLevelsChange, onAdditionalArchetypeChange, onArchetypeStackChange, onPrestigeSpellcastingTargetChange, onArchetypeChange, onAncestryChange, onAlignmentChange, onLevelChange, onReviewLevelUp, onSave, onLoad, onImport, onExport, onPrint, onReset }: {
  name: string;
  classId: string;
  ...
  onArchetypeStackChange: (classId: string, archetypeIds: string[]) => void;
  ...
  return <section className="builder" aria-label="Character details">
    ...
    <ArchetypePicker className={selectedClassName} characterClass={classes.find((item) => item.id === classId)} archetypes={primaryArchetypes} selectedIds={archetypeStacksByClass[classId] ?? (archetypeId ? [archetypeId] : [])} ancestryId={ancestryId} alignment={alignment} onChange={ids => { onArchetypeChange(ids[0] ?? ""); onArchetypeStackChange(classId, ids); }} />
    ...
    <label>Alignment<select aria-label="Alignment" value={alignment} onChange={(event) => onAlignmentChange(event.target.value as Alignment)}>{characterAlignments.map((item) => <option key={item} value={item}>{characterAlignmentLabel(item)}</option>)}</select></label>
