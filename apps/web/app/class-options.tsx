...
import type { AbilityName, ActiveEffect, CharacterOption } from "../../../packages/types/src/index.js";
...
const domainSpellLevel = (choice: Choice) => Number(choice.id.match(/^(?:cleric|druid|sacred-servant|faith-singer)-domain-spell-(\d+)$/)?.[1] ?? 0);
...
const choiceOrder = (choice: Choice) => {
  ...
  if (choice.id === "cleric-deity-1" || choice.id === "sacred-servant-deity-1" || choice.id === "faith-singer-deity-1" || isArcaneSchoolChoice(choice) || choice.id === "sorcerer-bloodline-1") return 10;
  if (choice.id === "paladin-divine-bond-5") return 11;
  if (choice.id === "cleric-alignment-1" || choice.id === "faith-singer-alignment-1" || choice.id.endsWith("opposition-school-1-first")) return 20;
  ...
  if (choice.id === "cleric-domain-1-first" || choice.id === "sacred-servant-domain-4" || choice.id === "faith-singer-domain-2") return 40;
...
const normalizedRuleName = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
const replacedNestedPower = (power: { name: string; level: number }, replacesText: string) => {
  const normalized = normalizedRuleName(replacesText);
  if (normalized.includes(normalizedRuleName(power.name))) return true;
  if (!/\b(?:domain|bloodline|school)\s+powers?\b|\brevelations?\b/.test(normalized)) return false;
  const levels = [...replacesText.matchAll(/\b(\d+)(?:st|nd|rd|th)\b/gi)].map((match) => Number(match[1]));
  return levels.length ? levels.includes(power.level) : true;
};

function OptionDetails({ option, archetypeReplacesText = "" }: { option: Option; archetypeReplacesText?: string }) {
  const bloodline = Boolean(option.arcana || option.bonusSpells || option.bonusFeats || option.classSkill);
  const visiblePowers = option.powers?.filter((power) => !replacedNestedPower(power, archetypeReplacesText));
  return <div className="option-details">
    ...
    {visiblePowers && visiblePowers.length > 0 && <div className="domain-powers"><strong>{bloodline ? "Bloodline powers" : "Granted powers"}</strong><ul>{visiblePowers.map((power) => <li key={`${power.level}-${power.name}`}><b>{power.name}</b> <small>level {power.level}</small><span>{power.summary}</span></li>)}</ul></div>}
...
export function ClassOptions({ choices, selectedOptions, classLevel, charismaModifier, abilityModifiers = {}, dailyResources = [], archetypeReplacesText = "", choiceChangeRules = {}, onOptionChange, onAddEffect }: { choices: Choice[]; selectedOptions: Record<string, string>; classLevel: number; charismaModifier: number; abilityModifiers?: Partial<Record<AbilityName, number>>; dailyResources?: DailyResource[]; archetypeReplacesText?: string; choiceChangeRules?: Record<string, ChoiceChangeRule>; onOptionChange: (featureId: string, optionId: string) => void; onAddEffect?: (effect: ActiveEffect) => void }) {
  ...
  const selectedDeity = orderedChoices.find((choice) => choice.id === "cleric-deity-1" || choice.id === "sacred-servant-deity-1" || choice.id === "faith-singer-deity-1")?.selected;
  const selectedAlignment = orderedChoices.find((choice) => choice.id === "cleric-alignment-1" || choice.id === "faith-singer-alignment-1")?.selected;
  ...
  const selectedDomains = orderedChoices.filter((choice) => choice.id === "cleric-domain-1-first" || choice.id === "cleric-domain-1-second" || choice.id === "druid-domain-1" || choice.id === "sacred-servant-domain-4" || choice.id === "faith-singer-domain-2").flatMap((choice) => choice.selected ? [choice.selected] : []);
  ...
  const optionsFor = (choice: Choice) => {
    ...
    if (choice.id === "cleric-alignment-1" || choice.id === "faith-singer-alignment-1") return alignmentsWithinOneStep(choice.options, selectedDeity?.alignment);
    ...
    if (choice.id === "sacred-servant-domain-4" || choice.id === "faith-singer-domain-2") return optionsGrantedBySelection(choice.options, selectedDeity);
  ...
  return <section className="choice-panel"><div><p className="eyebrow">FEATURE CHOICES</p><h2>Configure class features</h2><p>Dependent choices are ordered so each selection unlocks the next legal options.</p></div>{favoredTerrainBonuses.length > 0 && <aside className="favored-terrain-summary" aria-label="Favored terrain bonuses"><strong>Favored terrain bonuses</strong><ul>{favoredTerrainBonuses.map((terrain) => <li key={terrain.id}><span>{terrain.name}</span><b>+{terrain.bonus}</b></li>)}</ul></aside>}{domainSlotChoices.length > 0 && <button type="button" className="domain-slot-refresh" onClick={refreshDomainSlots}>Refresh domain spell slots</button>}{specialistSlotChoices.length > 0 && <button type="button" className="domain-slot-refresh" onClick={refreshSpecialistSlots}>Refresh specialist school slots</button>}{orderedChoices.map((choice) => {
    ...
    const needsDeity = (choice.id === "cleric-alignment-1" || choice.id === "faith-singer-alignment-1" || choice.id === "sacred-servant-domain-4" || choice.id === "faith-singer-domain-2" || (!domainLevel && choice.id.startsWith("cleric-domain-"))) && !selectedDeity;
    ...
    const changeRule = choiceChangeRules[choice.id];
    ...
    return <article className="choice-card" key={choice.id}><label>{choice.name} <small>level {choice.level}</small><select value={selectedOptions[choice.id] ?? ""} onChange={(event) => { if (selected?.choice) onOptionChange(detailKey, ""); onOptionChange(choice.id, event.target.value); if (trackedSpellSlot) onOptionChange(usedKey, ""); }} disabled={disabled}><option value="" disabled={Boolean(selected && changeRule?.preventClear)}>{placeholder}</option>{options.map((option) => <option key={option.id} value={option.id} disabled={!trackedSpellSlot && (domainSelection ? orderedChoices.some((other) => other.id !== choice.id && other.selected && domainEquivalent(other.selected) === domainEquivalent(option)) : conflictsWithSelectedFamily(choice, option) || selectedByOtherChoices(choice, option) >= optionSelectionLimit(option))}>{option.name}</option>)}</select></label>{changeRule && <small className="field-help">{changeRule.message}</small>}{selected && <OptionDetails option={selected} archetypeReplacesText={archetypeReplacesText} />}{selected?.resourceActions?.map((action) => {
      ...
      const maximumCost = Math.max(action.variableCost?.minimum ?? action.cost, Math.min(action.variableCost?.maximum ?? Number.POSITIVE_INFINITY, remaining));
      const cost = action.variableCost ? Math.max(action.variableCost.minimum, Math.min(actionAmounts[action.id] ?? action.variableCost.minimum, maximumCost)) : action.cost;
      const configuredEffects = action.activeEffects ?? (action.activeEffect ? [action.activeEffect] : []);
      const primaryEffect = configuredEffects[0];
      const bonus = action.activeEffect?.bonusAbilityModifier ? Math.max(0, abilityModifiers[action.activeEffect.bonusAbilityModifier] ?? 0) : primaryEffect?.bonus ?? 0;
      const rounds = primaryEffect?.durationAbilityModifier ? Math.max(0, abilityModifiers[primaryEffect.durationAbilityModifier] ?? 0) : primaryEffect?.defaultRounds ?? 1;
      const activate = () => {
        resource?.onUsedChange(resource.used + cost);
        configuredEffects.forEach((effect, index) => {
          const effectRounds = effect.durationAbilityModifier ? Math.max(0, abilityModifiers[effect.durationAbilityModifier] ?? 0) : effect.defaultRounds ?? 1;
          if (effectRounds > 0) onAddEffect?.({ id: `${action.id}-${index}-${Date.now()}-${Math.random()}`, name: effect.name, target: effect.target, bonus: action.activeEffect?.bonusAbilityModifier ? bonus : effect.bonus ?? 0, description: effect.description.replace("{cost}", String(cost)), roundsRemaining: effectRounds, ...(effect.d20Check ? { d20Check: { label: effect.d20Check.label, modifier: effect.d20Check.modifierClassLevel ? classLevel : 0, targetDc: effect.d20Check.targetDcDefault, ...(effect.d20Check.maximumSpellLevelFromCost ? { maximumSpellLevel: cost } : {}) } } : {}) });
        });
        setActionResults((current) => ({ ...current, [action.id]: primaryEffect && rounds > 0 ? `${primaryEffect.name} active for ${rounds} round${rounds === 1 ? "" : "s"}.` : "Ability used." }));
      };
      return <div className="option-resource-action" key={action.id}>{action.variableCost && <label>{action.variableCost.label}<input aria-label={`${action.label} ${action.variableCost.label.toLowerCase()}`} type="number" min={action.variableCost.minimum} max={maximumCost} value={cost} onChange={(event) => setActionAmounts((current) => ({ ...current, [action.id]: Math.max(action.variableCost!.minimum, Math.min(Number(event.target.value) || action.variableCost!.minimum, maximumCost)) }))} /></label>}<button type="button" disabled={!resource || remaining < cost || maximumCost < (action.variableCost?.minimum ?? action.cost) || (primaryEffect?.durationAbilityModifier !== undefined && rounds < 1)} onClick={activate}>{action.label}</button><small>{action.summary ?? `Spend ${cost} ${resource?.unit ?? "resource"}${cost === 1 ? "" : "s"}.`}</small>{actionResults[action.id] && <output aria-label={`${action.label} result`}>{actionResults[action.id]}</output>}</div>;
    })}{selected?.choice && <label>{selected.choice.label}{selected.choice.allowCustom ? <input aria-label={`${choice.name} ${selected.choice.label}`} value={selectedOptions[detailKey] ?? ""} onChange={(event) => onOptionChange(detailKey, event.target.value)} /> : <select aria-label={`${choice.name} ${selected.choice.label}`} value={selectedOptions[detailKey] ?? ""} onChange={(event) => onOptionChange(detailKey, event.target.value)}><option value="">Choose {selected.choice.label.toLowerCase()}</option>{(selected.choice.options ?? []).map((option) => <option key={option.id} value={option.id} disabled={usedDetailValues.includes(option.id)}>{option.name}</option>)}</select>}</label>}{choice.id === "sorcerer-bloodline-1" && selected?.classSkillChoices && selected.classSkillChoices.length > 0 && <label>Bloodline class skill<select aria-label="Bloodline class skill choice" value={selectedOptions[bloodlineSkillKey] ?? ""} onChange={(event) => onOptionChange(bloodlineSkillKey, event.target.value)}><option value="">Choose a Knowledge skill</option>{selected.classSkillChoices.map((skill) => <option key={skill} value={skill}>{skill}</option>)}</select></label>}{choice.id === "sorcerer-bloodline-1" && selected?.variants && selected.variants.length > 0 && <label>{isElementalBloodline ? "Element" : "Dragon type"}<select aria-label="Bloodline variant choice" value={selectedOptions[bloodlineVariantKey] ?? ""} onChange={(event) => onOptionChange(bloodlineVariantKey, event.target.value)}><option value="">{isElementalBloodline ? "Choose an element" : "Choose a dragon type"}</option>{selected.variants.map((variant) => <option key={variant.id} value={variant.id}>{variant.name}</option>)}</select></label>}{choice.id === "sorcerer-bloodline-1" && selectedBloodlineVariant && <p aria-label="Selected bloodline variant"><strong>{selectedBloodlineVariant.name}:</strong> {selectedBloodlineVariant.energyType}{variantDetail(selectedBloodlineVariant) && ` \u00b7 ${variantDetail(selectedBloodlineVariant)}`}</p>}{choice.id === "cleric-channel-energy-type-1" && selected && <ChannelEnergyTracker level={choice.classLevel ?? classLevel} charismaModifier={charismaModifier} used={channelUsed} onUsedChange={(next) => onOptionChange(channelUsedKey, String(next))} />}{trackedSpellSlot && selected && <div className="domain-slot-use"><output aria-label={`${choice.name} status`}>{used ? "Used" : "Available"}</output><button type="button" aria-label={`Cast ${selected.name} from ${choice.name}`} disabled={used} onClick={() => onOptionChange(usedKey, "used")}>{used ? "Used" : "Cast prepared spell"}</button></div>}</article>;
  })}</section>;
}
