...
export function ClassOptions({ choices, selectedOptions, classLevel, charismaModifier, dailyResources = [], archetypeReplacesText = "", onOptionChange }: { choices: Choice[]; selectedOptions: Record<string, string>; classLevel: number; charismaModifier: number; dailyResources?: DailyResource[]; archetypeReplacesText?: string; onOptionChange: (featureId: string, optionId: string) => void }) {
  ...
  return <section className="choice-panel"><div><p className="eyebrow">FEATURE CHOICES</p><h2>Configure class features</h2><p>Dependent choices are ordered so each selection unlocks the next legal options.</p></div>{domainSlotChoices.length > 0 && <button type="button" className="domain-slot-refresh" onClick={refreshDomainSlots}>Refresh domain spell slots</button>}{specialistSlotChoices.length > 0 && <button type="button" className="domain-slot-refresh" onClick={refreshSpecialistSlots}>Refresh specialist school slots</button>}{orderedChoices.map((choice) => {
    ...
    return <article className="choice-card" key={choice.id}><label>{choice.name} <small>level {choice.level}</small><select value={selectedOptions[choice.id] ?? ""} onChange={(event) => { if (selected?.choice) onOptionChange(detailKey, ""); onOptionChange(choice.id, event.target.value); if (trackedSpellSlot) onOptionChange(usedKey, ""); }} disabled={disabled}><option value="">{placeholder}</option>{options.map((option) => <option key={option.id} value={option.id} disabled={!trackedSpellSlot && (domainSelection ? orderedChoices.some((other) => other.id !== choice.id && other.selected && domainEquivalent(other.selected) === domainEquivalent(option)) : conflictsWithSelectedFamily(choice, option) || selectedByOtherChoices(choice, option) >= optionSelectionLimit(option))}>{option.name}</option>)}</select></label>{selected && <OptionDetails option={selected} archetypeReplacesText={archetypeReplacesText} />}{selected?.resourceActions?.map((action) => {
      ...
      return <div className="option-resource-action" key={action.id}><button type="button" disabled={!resource || remaining < action.cost} onClick={() => resource?.onUsedChange(resource.used + action.cost)}>{action.label}</button><small>{action.summary ?? `Spend ${action.cost} ${resource?.unit ?? "resource"}${action.cost === 1 ? "" : "s"}.`}</small></div>;
