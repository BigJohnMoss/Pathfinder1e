...
export function CombatPanel({ combat, conditionalModifiers = [] }: { combat: CombatStats; conditionalModifiers?: Array<{ label: string; bonus?: number; condition: string; source: string }> }) {
  return <article className="combat-panel"><p className="eyebrow">COMBAT</p><h2>Core statistics</h2><dl><div><dt>Initiative</dt><dd>{signed(combat.initiative)}</dd></div><div><dt>AC / touch / flat-footed</dt><dd>{combat.armorClass.normal} / {combat.armorClass.touch} / {combat.armorClass.flatFooted}</dd></div><div><dt>CMB / CMD</dt><dd>{signed(combat.combatManeuverBonus)} / {combat.combatManeuverDefense}</dd></div><div><dt>Average HP</dt><dd>{combat.averageHitPoints}</dd></div></dl>{conditionalModifiers.length > 0 && <section className="conditional-modifiers"><h3>Conditional trait modifiers</h3><ul>{conditionalModifiers.map((modifier, index) => <li key={`${modifier.source}-${modifier.label}-${index}`}><strong>{modifier.bonus === undefined ? modifier.label : `${signed(modifier.bonus)} ${modifier.label}`}</strong><span>{modifier.condition} · {modifier.source}</span></li>)}</ul></section>}<p className="hint">Average HP uses maximum 1st-level hit points and rounded-up average rolls thereafter. Conditional modifiers are listed separately so they are only applied when their trigger is present.</p></article>;
}
