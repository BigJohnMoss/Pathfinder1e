...
export function archetypeArmorConditionedBenefits(archetypes: CharacterArchetype[], classLevels: Record<string, number>, context?: { armorCategory?: "none" | "light" | "medium" | "heavy"; shieldBonus?: number; selectedOptions?: Record<string, string> }): { armorClass: { normal: number; touch: number; flatFooted: number }; defenses: Array<{ sourceFeatureId: string; kind: "damageReduction" | "fortification" | "immunity"; label: string; value: number; qualifier: string; condition?: string; source: string }>; conditionalModifiers: Array<{ label: string; bonus?: number; condition: string; source: string }>; handledFeatureIds: string[]; suppressedConditionalModifierLabels: string[] };
...
export function archetypeWeaponProficiencyRules(archetypes: CharacterArchetype[], selectedOptions?: Record<string, string>): Array<{ sourceFeatureId?: string; label: string; proficiencies: string[]; condition?: string; source: string }>;
...
export function inferArchetypeResourceAdjustments(archetype?: CharacterArchetype): Array<{ resourceId: string; label: string; unit: string; operation: "replace"; minimumLevel: number; base: number; perInterval?: number; interval?: number; levelDivisor?: number; levelMultiplier?: number; abilityModifier?: AbilityName; abilityMultiplier?: number; minimum?: number; maximum?: number; maximumByLevel?: Array<{ level: number; maximum: number }>; effectiveLevelClassIds?: string[] }>;
