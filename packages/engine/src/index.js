...
export function archetypeAbilityScoreBonuses(archetypes = [], classLevels = {}) {
  const result = Object.fromEntries(["strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"].map((ability) => [ability, 0]));
  for (const ability of Object.keys(result)) {
    let stackable = 0;
    const typed = new Map();
    for (const archetype of archetypes ?? []) {
      const level = archetypeLevel(archetype, classLevels);
      for (const adjustment of archetypeAbilityScoreAdjustments(archetype)) {
        if (adjustment.ability !== ability || adjustment.condition || !adjustmentAppliesAtLevel(adjustment, level)) continue;
        const bonus = adjustmentBonusAtLevel(adjustment, level);
        if (!adjustment.bonusType || ["circumstance", "untyped"].includes(adjustment.bonusType)) stackable += bonus;
        else typed.set(adjustment.bonusType, Math.max(typed.get(adjustment.bonusType) ?? 0, bonus));
      }
    }
    result[ability] = stackable + [...typed.values()].reduce((total, bonus) => total + bonus, 0);
  }
  return result;
}
          ...
          .filter(
            (effect) =>
              ...
              [
                ...
                "casterLevelChecks",
                "savingThrows",
                "savingThrowsAgainstCharmAndFear",
                "savingThrowsAgainstParalysisAndSleep",
                "meleeDamageRolls",
                "healingReceived",
                "skillChecks",
                "strength",
...
export function applyArchetype(characterClass, archetype, referenceClasses = [], spellCatalog = []) {
  ...
  const spellcastingAbility = archetype.spellcastingAbility
    ?? (inferredSpellcastingAbility && (inferredSpellcastingAbility.directAbility || inferredSpellcastingAbility.replacesAbility === characterSpellcasting?.ability)
  ...
  const inferredClassSkillDetails = inferArchetypeClassSkillDetails(archetype);
  const inferredClassSkills = hasStructuredClassSkills
    ? { additions: [], removals: [] }
    : { additions: inferredClassSkillDetails.additions, removals: inferredClassSkillDetails.removals };
  const inferredClassSkillReplacement = hasStructuredClassSkills ? [] : inferredClassSkillDetails.replacement;
  ...
  return {
    ...
    spellcasting: baseSpellcasting
      ? {
          ...
          castingType: inferredSpellcastingAbility?.castingType ?? baseSpellcasting.castingType,
          tradition: inferredSpellcastingAbility?.tradition ?? spellcastingTradition(characterClass) ?? baseSpellcasting.tradition,
    ...
    classSkills: inferredClassSkillReplacement.length ? inferredClassSkillReplacement : [
      ...new Set(
...
function inferArchetypeClassSkillDetails(archetype) {
  ...
  const hasStructuredChanges = (archetype?.classSkillAdditions?.length ?? 0) + (archetype?.classSkillRemovals?.length ?? 0) > 0;
...
export function inferArchetypeClassSkillChanges(archetype) {
  const { additions, removals } = inferArchetypeClassSkillDetails(archetype);
...
function namedArchetypeProficiencies(fragment) {
  ...
  if (/\ball armors? and shields?\b/i.test(text)) add("shield", "All shields");
  ...
  if (/\bshields?\b/i.test(text) && (!/\b(?:bucklers?|light shields?|heavy shields?|tower shields?)\b/i.test(text) || /\bshields?\s+(?:but not|except)/i.test(text)))
...
function inferArchetypeProficiencyDetails(archetype, includeFullyAutomatedFeatureIds = true) {
  ...
  const adjustments = [...grouped.entries()].map(([key, proficiencies]) => {
...
export function inferArchetypeProficiencyAdjustments(archetype) {
  return inferArchetypeProficiencyDetails(archetype).adjustments;
}
...
export function archetypeAutomationSummary(archetype, feats = [], spells = []) {
  ...
  if (inferredSpellcastingAbility?.castingType) automated.push(`Casting type: ${inferredSpellcastingAbility.castingType}`);
  if (inferredSpellcastingAbility?.tradition) automated.push(`Spellcasting tradition: ${inferredSpellcastingAbility.tradition}`);
  ...
  const classSkillDetails = inferArchetypeClassSkillDetails(archetype);
  const inferredClassSkills = { additions: classSkillDetails.additions, removals: classSkillDetails.removals };
  ...
  const resourceAdjustments = resolvedArchetypeResourceAdjustments(archetype);
  ...
  const resourceDamageActionDetails = inferredArchetypeResourceDamageActionDetails(archetype);
  if (resourceDamageActionDetails.actions.length)
    automated.push(`${resourceDamageActionDetails.actions.length} calculated resource-damage action${resourceDamageActionDetails.actions.length === 1 ? "" : "s"}`);
  ...
  const adjustmentFeatureIds = new Set([
    ...
    ...(inferredSpellcastingAbility?.fullyAutomatedFeatureIds ?? []),
