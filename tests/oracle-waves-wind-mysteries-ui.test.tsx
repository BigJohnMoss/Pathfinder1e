...
test("Waves and Wind expose their complete selectable and persistent progressions", async () => {
  ...
  await user.selectOptions(mystery, "oracle-mystery-waves");
  assert.ok(screen.getByText("tsunami"));
  await user.selectOptions(revelation, "oracle-revelation-blizzard");

  await user.selectOptions(mystery, "oracle-mystery-wind");
  await waitFor(() => assert.equal(revelation.value, ""));
  assert.ok(screen.getByText("winds of vengeance"));
  await user.selectOptions(revelation, "oracle-revelation-wings-of-air");

  await user.click(screen.getByRole("tab", { name: "Spells" }));
  assert.equal(screen.getByLabelText("winds of vengeance known").textContent, "Mystery");
  ...
  await user.click(screen.getByRole("tab", { name: "Features" }));
  assert.equal(selectFor("Mystery").value, "oracle-mystery-wind");
  assert.equal(selectFor("Revelation", 0).value, "oracle-revelation-wings-of-air");
});
