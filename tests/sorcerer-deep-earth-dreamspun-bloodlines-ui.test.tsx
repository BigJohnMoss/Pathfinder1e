...
test("Deep Earth and Dreamspun expose details, granted spells, switching, and persistence", async () => {
  ...
  await user.selectOptions(optionSelect("Bloodline"), "sorcerer-bloodline-deep-earth");
  assert.ok(screen.getByText("Tremor"));
  assert.ok(screen.getByText("Strength of Stone"));
  ...
  assert.equal(screen.getByLabelText("Expeditious Excavation known").textContent, "Bloodline");

  await user.click(screen.getByRole("tab", { name: "Features" }));
  await user.selectOptions(optionSelect("Bloodline"), "sorcerer-bloodline-dreamspun");
  assert.ok(screen.getByText("Lullaby"));
  assert.ok(screen.getByText("Solipsism"));
  assert.ok(screen.getByText(/Astral Projection/));
  await user.click(screen.getByRole("tab", { name: "Spells" }));
  await user.selectOptions(screen.getByLabelText("Spell level filter"), "1");
  assert.equal(screen.getByLabelText("Sleep known").textContent, "Bloodline");
  assert.equal(screen.getByLabelText("Expeditious Excavation known").textContent, "Unknown");
  ...
  await user.click(screen.getByRole("tab", { name: "Features" }));
  assert.equal(optionSelect("Bloodline").value, "sorcerer-bloodline-dreamspun");
});
