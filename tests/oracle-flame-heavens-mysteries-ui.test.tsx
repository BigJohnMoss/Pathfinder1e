...
test("Flame and Heavens expose grants, restrictions, switching, and persistence", async () => {
  ...
  const curse = selectFor("Oracle's Curse");
  const revelation = selectFor("Revelation", 0);
  await user.selectOptions(mystery, "oracle-mystery-flame");
  assert.ok(screen.getByText("fiery body"));
  assert.ok(Array.from(revelation.options).some((option) => option.value === "oracle-revelation-cinder-dance"));
  await user.selectOptions(curse, "oracle-curse-lame");
  await waitFor(() => assert.equal(Array.from(revelation.options).some((option) => option.value === "oracle-revelation-cinder-dance"), false));
  await user.selectOptions(revelation, "oracle-revelation-firestorm");

  await user.selectOptions(mystery, "oracle-mystery-heavens");
  await waitFor(() => assert.equal(revelation.value, ""));
  assert.ok(screen.getByText("meteor swarm"));
  await user.selectOptions(revelation, "oracle-revelation-dweller-in-darkness");

  await user.click(screen.getByRole("tab", { name: "Spells" }));
  assert.equal(screen.getByLabelText("meteor swarm known").textContent, "Mystery");
  ...
  await user.click(screen.getByRole("tab", { name: "Features" }));
  assert.equal(selectFor("Mystery").value, "oracle-mystery-heavens");
  assert.equal(selectFor("Revelation", 0).value, "oracle-revelation-dweller-in-darkness");
});
