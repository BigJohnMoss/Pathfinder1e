...
test("Shadow and Starsoul expose details, granted spells, switching, and persistence", async () => {
  ...
  await user.selectOptions(optionSelect("Bloodline"), "sorcerer-bloodline-shadow");
  assert.ok(screen.getByText("Shadowstrike"));
  assert.ok(screen.getByText("Shadow Master"));
  assert.ok(screen.getByText(/Shades/));
  await user.click(screen.getByRole("tab", { name: "Spells" }));
  await user.selectOptions(screen.getByLabelText("Spell level filter"), "1");
  assert.equal(screen.getByLabelText("Ray of Enfeeblement known").textContent, "Bloodline");

  await user.click(screen.getByRole("tab", { name: "Features" }));
  await user.selectOptions(optionSelect("Bloodline"), "sorcerer-bloodline-starsoul");
  assert.ok(screen.getByText("Minute Meteors"));
  assert.ok(screen.getByText("Starborn"));
  assert.ok(screen.getByText(/Meteor Swarm/));
  await user.click(screen.getByRole("tab", { name: "Spells" }));
  await user.selectOptions(screen.getByLabelText("Spell level filter"), "1");
  assert.equal(screen.getByLabelText("Unseen Servant known").textContent, "Bloodline");
  assert.equal(screen.getByLabelText("Ray of Enfeeblement known").textContent, "Unknown");
  ...
  await user.click(screen.getByRole("tab", { name: "Features" }));
  assert.equal(optionSelect("Bloodline").value, "sorcerer-bloodline-starsoul");
});
