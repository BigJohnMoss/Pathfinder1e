...
test("uses equipped attacks and quick hit point controls during combat", async () => {
  ...
  assert.ok(screen.getByLabelText("Initiative total"));
  ...
  assert.ok(screen.getByLabelText("Perception total"));
  fireEvent.change(screen.getByLabelText("Custom dice count"), { target: { value: "2" } });
  await user.selectOptions(screen.getByLabelText("Custom die sides"), "6");
...
test("selects, applies, and persists traits from different categories", async () => {
  ...
  assert.match(screen.getByText("Core statistics").closest("article")?.textContent ?? "", /Initiative\+2/);
