...
test("opposition-school spells cost two prepared slots", () => {
  assert.equal(spellPreparationCost(spells[0], ["wizard-school-conjuration"]), 1);
  assert.equal(spellPreparationCost(spells[1], ["wizard-school-conjuration"]), 2);
  assert.equal(spellPreparationCost(spells[2], ["wizard-school-abjuration"]), 2);
...
test("prepared spell usage counts opposition costs by spell level", () => {
  assert.deepEqual(preparedSpellSlotUsage(["mage-armor", "detect-magic", "detect-magic"], spells, "wizard", ["wizard-school-conjuration"]), { 0: 2, 1: 2 });
  assert.deepEqual(preparedSpellSlotUsage(["missing"], spells, "wizard", ["wizard-school-conjuration"]), {});
});

test("normalization enforces weighted prepared capacity in selection order", () => {
  assert.deepEqual(normalizePreparedSpellsWithOpposition(["mage-armor", "magic-missile"], spells, "wizard", limits, ["wizard-school-conjuration"]), ["mage-armor"]);
  assert.deepEqual(normalizePreparedSpellsWithOpposition(["magic-missile", "mage-armor"], spells, "wizard", limits, ["wizard-school-conjuration"]), ["magic-missile"]);
  assert.deepEqual(normalizePreparedSpellsWithOpposition(["magic-missile", "magic-missile"], spells, "wizard", limits, ["wizard-school-conjuration"]), ["magic-missile", "magic-missile"]);
...
test("normalization rejects invalid and over-capacity records safely", () => {
  assert.deepEqual(normalizePreparedSpellsWithOpposition(["missing", "detect-magic", "detect-magic", "detect-magic", "detect-magic"], spells, "wizard", limits, ["wizard-school-divination"]), ["detect-magic"]);
