...
test("Aquatic and Boreal expose complete details, granted spells, switching, and persistence", async () => {
  ...
  await user.selectOptions(optionSelect("Bloodline"), "sorcerer-bloodline-aquatic");
  assert.ok(screen.getByText("Dehydrating Touch"));
  assert.ok(screen.getByText("Deep One"));
  ...
  assert.equal(screen.getByLabelText("Hydraulic Push known").textContent, "Bloodline");

  await user.click(screen.getByRole("tab", { name: "Features" }));
  await user.selectOptions(optionSelect("Bloodline"), "sorcerer-bloodline-boreal");
  assert.ok(screen.getByText("Cold Steel"));
  assert.ok(screen.getByText("Child of Ancient Winters"));
  assert.ok(screen.getByText(/Meteor Swarm/));
  await user.click(screen.getByRole("tab", { name: "Spells" }));
  await user.selectOptions(screen.getByLabelText("Spell level filter"), "1");
  assert.equal(screen.getByLabelText("Enlarge Person known").textContent, "Bloodline");
  assert.equal(screen.getByLabelText("Hydraulic Push known").textContent, "Unknown");
  ...
  await user.click(screen.getByRole("tab", { name: "Features" }));
  assert.equal(optionSelect("Bloodline").value, "sorcerer-bloodline-boreal");
});
