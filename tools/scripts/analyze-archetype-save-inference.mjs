...
import { archetypeAutomationSummary, archetypeSaveBonusAdjustments } from "../../packages/engine/src/index.js";
import { inferredArchetypeSaveBonusDetails } from "../../packages/engine/src/archetype-saves.js";

const rows = [];
const runtimeRows = [];
for (const archetype of archetypes) {
  const details = inferredArchetypeSaveBonusDetails(archetype);
  const runtime = new Set(archetypeSaveBonusAdjustments(archetype).map((adjustment) => JSON.stringify(adjustment)));
...
console.log(`Inferred save adjustments: ${rows.length}`);
...
console.log(`Permanent save adjustments: ${runtimeRows.filter((row) => !row.adjustment.condition).length}`);
