...
const activeEffectTargets = new Set(["initiative", "armorClass", "fortitude", "reflex", "will", "attackRolls", "meleeAttackRolls", "damageRolls", "weaponDamageRolls", "spellResistance", "casterLevel", "spellSaveDc", "exploitEffectiveLevel", "casterLevelChecks", "savingThrows", "savingThrowsAgainstCharmAndFear", "savingThrowsAgainstParalysisAndSleep", "meleeDamageRolls", "healingReceived", "skillChecks", "performanceChecks", "performanceSaveDc", "landSpeed", ...abilities, "allies", "self", "area", "enemy"]);
const invalidModeEffect = (effect) => !effect || !activeEffectTargets.has(effect.target) || !Number.isInteger(effect.bonus) || effect.bonus < -20 || effect.bonus > 20 || typeof effect.label !== "string" || !effect.label.trim() || typeof effect.description !== "string" || !effect.description.trim() || (effect.fastHealing !== undefined && (!Number.isInteger(effect.fastHealing) || effect.fastHealing < 1 || effect.fastHealing > 999)) || (effect.bonusByLevel !== undefined && (!Array.isArray(effect.bonusByLevel) || effect.bonusByLevel.length === 0 || effect.bonusByLevel.some((step, index) => !Number.isInteger(step?.level) || step.level < 1 || step.level > 20 || !Number.isInteger(step.bonus) || step.bonus < -20 || step.bonus > 20 || (index > 0 && step.level <= effect.bonusByLevel[index - 1].level)))) || (effect.skillIds !== undefined && (!Array.isArray(effect.skillIds) || effect.skillIds.length === 0 || new Set(effect.skillIds).size !== effect.skillIds.length || effect.skillIds.some((skill) => typeof skill !== "string" || !skill.trim())));
...
for (const [index, url] of archetypeUrls.entries()) {
  ...
  for(const replacement of archetype.replacements??[]) {
    ...
    for(const feature of replacement.features??[]) {
      ...
      if (feature.channelEnergyDiceAdvancementOptionIds !== undefined) {
        if (!Array.isArray(feature.channelEnergyDiceAdvancementOptionIds) || feature.channelEnergyDiceAdvancementOptionIds.length === 0 || new Set(feature.channelEnergyDiceAdvancementOptionIds).size !== feature.channelEnergyDiceAdvancementOptionIds.length || feature.channelEnergyDiceAdvancementOptionIds.some(id => typeof id !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id))) errors.push(`${file}: ${feature.id} has invalid channelEnergyDiceAdvancementOptionIds`);
        else for (const optionId of feature.channelEnergyDiceAdvancementOptionIds) featureRequiredOptionRefs.push({ file, featureId: feature.id, itemId: "channel energy die advancement", optionId });
      }
      if (feature.channelEnergyPolarityOptionIds !== undefined) {
        const polarities = feature.channelEnergyPolarityOptionIds;
        if (!polarities || typeof polarities !== "object" || Array.isArray(polarities) || Object.keys(polarities).sort().join(",") !== "negative,positive" || Object.values(polarities).some(id => typeof id !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id)) || polarities.positive === polarities.negative) errors.push(`${file}: ${feature.id} has invalid channelEnergyPolarityOptionIds`);
        else for (const optionId of Object.values(polarities)) featureRequiredOptionRefs.push({ file, featureId: feature.id, itemId: "channel energy polarity", optionId });
      }
      for (const action of feature.resourceActions ?? []) {
        ...
        if (action.maximumLevel !== undefined && (!Number.isInteger(action.maximumLevel) || action.maximumLevel < (action.minimumLevel ?? feature.level) || action.maximumLevel > 20)) errors.push(`${file}: ${feature.id} ${action.id} has an invalid maximumLevel`);
        ...
        if (action.targetCountByLevel !== undefined && (!Array.isArray(action.targetCountByLevel) || action.targetCountByLevel.length === 0 || action.targetCountByLevel[0]?.level !== (action.minimumLevel ?? feature.level) || action.targetCountByLevel.some((step, index) => !Number.isInteger(step?.level) || step.level < (action.minimumLevel ?? feature.level) || step.level > 20 || !Number.isInteger(step.count) || step.count < 1 || step.count > 999 || (index > 0 && step.level <= action.targetCountByLevel[index - 1].level)))) errors.push(`${file}: ${feature.id} ${action.id} has invalid targetCountByLevel`);
        if (action.maximumActiveEffects !== undefined) {
          const limit = action.maximumActiveEffects;
          if (!limit || typeof limit.name !== "string" || !limit.name.trim() || (limit.base !== undefined && (!Number.isInteger(limit.base) || limit.base < 0)) || (limit.levelDivisor !== undefined && (!Number.isInteger(limit.levelDivisor) || limit.levelDivisor < 1 || limit.levelDivisor > 20)) || (limit.abilityModifier !== undefined && !abilities.has(limit.abilityModifier)) || (limit.minimum !== undefined && (!Number.isInteger(limit.minimum) || limit.minimum < 0)) || (limit.maximum !== undefined && (!Number.isInteger(limit.maximum) || limit.maximum < (limit.minimum ?? 0) || limit.maximum > 999))) errors.push(`${file}: ${feature.id} ${action.id} has invalid maximumActiveEffects`);
        }
        if (action.variableCost !== undefined) {
          const variable = action.variableCost;
          const minimumLevel = action.minimumLevel ?? feature.level;
          if (!variable || typeof variable.label !== "string" || !variable.label.trim() || !Number.isInteger(variable.minimum) || variable.minimum < 1 || (variable.maximum !== undefined && (!Number.isInteger(variable.maximum) || variable.maximum < variable.minimum || variable.maximum > 999)) || (variable.maximumLevelDivisor !== undefined && (!Number.isInteger(variable.maximumLevelDivisor) || variable.maximumLevelDivisor < 1 || variable.maximumLevelDivisor > 20)) || (variable.multiplier !== undefined && (!Number.isInteger(variable.multiplier) || variable.multiplier < 1 || variable.multiplier > 999))) errors.push(`${file}: ${feature.id} ${action.id} has an invalid variableCost`);
          if (variable?.maximumByLevel !== undefined && (!Array.isArray(variable.maximumByLevel) || variable.maximumByLevel.length === 0 || variable.maximumByLevel[0]?.level !== minimumLevel || variable.maximumByLevel.some((step, index) => !Number.isInteger(step?.level) || step.level < minimumLevel || step.level > 20 || !Number.isInteger(step.maximum) || step.maximum < variable.minimum || step.maximum > 999 || (index > 0 && step.level <= variable.maximumByLevel[index - 1].level)))) errors.push(`${file}: ${feature.id} ${action.id} has invalid variableCost maximumByLevel`);
        }
        ...
        if (action.modes !== undefined && (!Array.isArray(action.modes) || action.modes.length === 0 || new Set(action.modes.map(mode => mode?.id)).size !== action.modes.length || action.modes.some(mode => !mode || typeof mode.id !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(mode.id) || typeof mode.label !== "string" || !mode.label.trim() || typeof mode.summary !== "string" || !mode.summary.trim() || (mode.requiredOptionId !== undefined && (typeof mode.requiredOptionId !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(mode.requiredOptionId))) || (mode.activeEffects !== undefined && (!Array.isArray(mode.activeEffects) || mode.activeEffects.length === 0 || mode.activeEffects.some(invalidModeEffect)))))) errors.push(`${file}: ${feature.id} ${action.id} has invalid modes`);
        else for (const mode of action.modes ?? []) if (mode.requiredOptionId) featureRequiredOptionRefs.push({ file, featureId: feature.id, itemId: `${action.id} mode ${mode.id}`, optionId: mode.requiredOptionId });
        if (action.diceRoll?.diceCountBonusOptionIds !== undefined) {
          const optionIds = action.diceRoll.diceCountBonusOptionIds;
          if (!Array.isArray(optionIds) || optionIds.length === 0 || new Set(optionIds).size !== optionIds.length || optionIds.some(id => typeof id !== "string" || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id))) errors.push(`${file}: ${feature.id} ${action.id} has invalid diceCountBonusOptionIds`);
          else for (const optionId of optionIds) featureRequiredOptionRefs.push({ file, featureId: feature.id, itemId: `${action.id} dice bonus`, optionId });
        }
        if (action.fixedD20Result !== undefined) {
          const fixed = action.fixedD20Result;
          const steps = fixed?.resultByLevel;
          const invalidSteps = steps !== undefined && (!Array.isArray(steps) || steps.length === 0 || steps.some((step, index) => !Number.isInteger(step?.level) || step.level < (action.minimumLevel ?? feature.level) || step.level > 20 || ![10, 20].includes(step.result) || index > 0 && step.level <= steps[index - 1].level));
          if (!fixed || typeof fixed.label !== "string" || !fixed.label.trim() || ![10, 20].includes(fixed.result) || invalidSteps) errors.push(`${file}: ${feature.id} ${action.id} has an invalid fixedD20Result`);
        }
        if (action.diceRoll?.modifierInputLabel !== undefined && (typeof action.diceRoll.modifierInputLabel !== "string" || !action.diceRoll.modifierInputLabel.trim())) errors.push(`${file}: ${feature.id} ${action.id} has an invalid dice modifier input label`);
        if (action.diceRoll?.targetDcInputLabel !== undefined && (typeof action.diceRoll.targetDcInputLabel !== "string" || !action.diceRoll.targetDcInputLabel.trim())) errors.push(`${file}: ${feature.id} ${action.id} has an invalid dice target DC input label`);
        if (action.diceRoll?.outcomesByMargin !== undefined && (!action.diceRoll.targetDcInputLabel || !Array.isArray(action.diceRoll.outcomesByMargin) || action.diceRoll.outcomesByMargin.length === 0 || new Set(action.diceRoll.outcomesByMargin.map((outcome) => outcome?.minimumMargin)).size !== action.diceRoll.outcomesByMargin.length || action.diceRoll.outcomesByMargin.some((outcome) => !Number.isInteger(outcome?.minimumMargin) || outcome.minimumMargin < -999 || outcome.minimumMargin > 999 || typeof outcome.label !== "string" || !outcome.label.trim()))) errors.push(`${file}: ${feature.id} ${action.id} has invalid dice outcomesByMargin`);
        if (action.diceRoll?.outcomesByTotal !== undefined) {
          const modeIds = new Set(action.modes?.map((mode) => mode.id) ?? []);
          const outcomes = action.diceRoll.outcomesByTotal;
          const invalidOutcome = (outcome, index) => !outcome || !Number.isInteger(outcome.minimumTotal) || outcome.minimumTotal < -999 || outcome.minimumTotal > 9999 || (outcome.maximumTotal !== undefined && (!Number.isInteger(outcome.maximumTotal) || outcome.maximumTotal < outcome.minimumTotal || outcome.maximumTotal > 9999)) || index > 0 && outcome.minimumTotal <= outcomes[index - 1].minimumTotal || typeof outcome.label !== "string" || !outcome.label.trim() || (outcome.summary !== undefined && (typeof outcome.summary !== "string" || !outcome.summary.trim())) || (outcome.effectsByMode !== undefined && (!Array.isArray(outcome.effectsByMode) || outcome.effectsByMode.length === 0 || outcome.effectsByMode.some((effect) => !effect || !modeIds.has(effect.modeId) || typeof effect.name !== "string" || !effect.name.trim() || !activeEffectTargets.has(effect.target) || !Number.isInteger(effect.bonus) || effect.bonus < -20 || effect.bonus > 20 || typeof effect.description !== "string" || !effect.description.trim() || !Number.isInteger(effect.rounds) || effect.rounds < 1 || effect.rounds > 999)));
          if (!Array.isArray(outcomes) || outcomes.length === 0 || outcomes.some(invalidOutcome)) errors.push(`${file}: ${feature.id} ${action.id} has invalid dice outcomesByTotal`);
        }
        if (action.diceRoll?.failureLabel !== undefined && (!action.diceRoll.targetDcInputLabel || typeof action.diceRoll.failureLabel !== "string" || !action.diceRoll.failureLabel.trim())) errors.push(`${file}: ${feature.id} ${action.id} has an invalid dice failureLabel`);
        if (action.diceRoll?.flatModifierByLevel !== undefined && (!Array.isArray(action.diceRoll.flatModifierByLevel) || action.diceRoll.flatModifierByLevel.length === 0 || action.diceRoll.flatModifierByLevel[0]?.level !== (action.minimumLevel ?? feature.level) || action.diceRoll.flatModifierByLevel.some((step, index) => !Number.isInteger(step?.level) || step.level < (action.minimumLevel ?? feature.level) || step.level > 20 || !Number.isInteger(step.modifier) || step.modifier < -999 || step.modifier > 999 || (index > 0 && step.level <= action.diceRoll.flatModifierByLevel[index - 1].level)))) errors.push(`${file}: ${feature.id} ${action.id} has invalid dice flatModifierByLevel`);
        if (action.diceRoll?.resultDivisorByMode !== undefined) {
          const modeIds = new Set(action.modes?.map((mode) => mode.id) ?? []);
          if (!Array.isArray(action.diceRoll.resultDivisorByMode) || action.diceRoll.resultDivisorByMode.length === 0 || new Set(action.diceRoll.resultDivisorByMode.map((step) => step?.modeId)).size !== action.diceRoll.resultDivisorByMode.length || action.diceRoll.resultDivisorByMode.some((step) => !step || !modeIds.has(step.modeId) || !Number.isInteger(step.divisor) || step.divisor < 2 || step.divisor > 100)) errors.push(`${file}: ${feature.id} ${action.id} has invalid dice resultDivisorByMode`);
        }
        ...
        if (action.targetEffectRoll !== undefined) {
          const roll = action.targetEffectRoll;
          const minimumLevel = action.minimumLevel ?? feature.level;
          const validDuration = (duration) => duration && ((duration.kind === "fixed-rounds" && Number.isInteger(duration.rounds) && duration.rounds >= 1 && duration.rounds <= 999) || (duration.kind === "dice-rounds" && Number.isInteger(duration.count) && duration.count >= 1 && duration.count <= 100 && Number.isInteger(duration.sides) && duration.sides >= 2 && duration.sides <= 1000) || duration.kind === "level-rounds" || (["level-minutes", "level-hours"].includes(duration.kind) && (duration.multiplier === undefined || Number.isInteger(duration.multiplier) && duration.multiplier >= 1 && duration.multiplier <= 100)));
          const validActiveEffects = (effects) => effects === undefined || Array.isArray(effects) && effects.length > 0 && effects.every((effect) => effect && activeEffectTargets.has(effect.target) && Number.isInteger(effect.bonus) && effect.bonus >= -20 && effect.bonus <= 20 && typeof effect.label === "string" && effect.label.trim() && typeof effect.description === "string" && effect.description.trim());
          if (!roll || !["fortitude", "reflex", "will"].includes(roll.modifier) || !action.savingThrow || !Array.isArray(roll.effectsByLevel) || roll.effectsByLevel.length === 0 || roll.effectsByLevel[0]?.level !== minimumLevel || roll.effectsByLevel.some((effect, index) => !Number.isInteger(effect?.level) || effect.level < minimumLevel || effect.level > 20 || index > 0 && effect.level <= roll.effectsByLevel[index - 1].level || typeof effect.name !== "string" || !effect.name.trim() || typeof effect.description !== "string" || !effect.description.trim() || !validDuration(effect.duration) || !validActiveEffects(effect.activeEffects))) errors.push(`${file}: ${feature.id} ${action.id} has an invalid targetEffectRoll`);
        }
        ...
        if (action.rerollAction !== undefined && (!action.rerollAction || !["d20", "damage", "lower-d20", "higher-d20"].includes(action.rerollAction.kind) || typeof action.rerollAction.label !== "string" || !action.rerollAction.label.trim())) errors.push(`${file}: ${feature.id} ${action.id} has an invalid rerollAction`);
        if (action.combatRoll !== undefined) {
          const roll = action.combatRoll;
          const combatMinimumLevel = action.minimumLevel ?? feature.level;
          const validSteps = (steps, key, allowed) => Array.isArray(steps) && steps.length > 0 && [feature.level, combatMinimumLevel].includes(steps[0]?.level) && steps.every((step, index) => Number.isInteger(step?.level) && step.level >= feature.level && step.level <= 20 && allowed(step?.[key]) && (index === 0 || step.level > steps[index - 1].level));
        ...
        if (action.activeEffect) {
          ...
          const activeEffectMinimumLevel = action.minimumLevel ?? feature.level;
          if (effect.bonusByLevel !== undefined && (!Array.isArray(effect.bonusByLevel) || effect.bonusByLevel.length === 0 || effect.bonusByLevel[0]?.level < feature.level || effect.bonusByLevel[0]?.level > activeEffectMinimumLevel || effect.bonusByLevel.some((step, index) => !Number.isInteger(step?.level) || step.level < feature.level || step.level > 20 || !Number.isInteger(step.bonus) || step.bonus < -20 || step.bonus > 99 || (index > 0 && step.level <= effect.bonusByLevel[index - 1].level)))) errors.push(`${file}: ${feature.id} ${action.id} has invalid activeEffect bonusByLevel`);
          ...
          if (effect.durationDice !== undefined && (!effect.durationDice || !Number.isInteger(effect.durationDice.count) || effect.durationDice.count < 1 || effect.durationDice.count > 100 || !Number.isInteger(effect.durationDice.sides) || effect.durationDice.sides < 2 || effect.durationDice.sides > 1000 || effect.defaultRounds !== undefined || effect.defaultRoundsByLevel !== undefined)) errors.push(`${file}: ${feature.id} ${action.id} has invalid activeEffect durationDice`);
      ...
      for (const calculation of feature.numericCalculations ?? []) {
        ...
        if (calculation?.inputMultiplierByLevel !== undefined && (!Array.isArray(calculation.inputMultiplierByLevel) || calculation.inputMultiplierByLevel.length === 0 || calculation.inputMultiplierByLevel[0]?.level !== feature.level || calculation.inputMultiplierByLevel.some((step, index) => !Number.isInteger(step?.level) || step.level < feature.level || step.level > 20 || !Number.isInteger(step.multiplier) || step.multiplier < -999 || step.multiplier > 999 || (index > 0 && step.level <= calculation.inputMultiplierByLevel[index - 1].level)))) errors.push(`${file}: ${feature.id} has an invalid numeric calculation multiplier`);
      }
  ...
  for (const adjustment of archetype.defenseAdjustments ?? []) {
    ...
    if (adjustment?.sourceFeatureId !== undefined && !archetype.replacements?.some(replacement => replacement.features?.some(feature => feature.id === adjustment.sourceFeatureId))) errors.push(`${prefix} references unknown sourceFeatureId ${adjustment.sourceFeatureId}`);
    if (!["damageReduction", "energyResistance", "spellResistance", "immunity", "evasion", "improvedEvasion", "uncannyDodge", "improvedUncannyDodge", "fortification", "concealment", "missChance", "fastHealing", "regeneration"].includes(adjustment?.kind)) errors.push(`${prefix} has an invalid kind`);
