  ...
  for(const replacement of archetype.replacements??[]) {
    ...
    for(const feature of replacement.features??[]) {
      ...
      for (const action of feature.resourceActions ?? []) {
        ...
        if (action.fixedD20Result !== undefined && (!action.fixedD20Result || typeof action.fixedD20Result.label !== "string" || !action.fixedD20Result.label.trim() || ![10, 20].includes(action.fixedD20Result.result))) errors.push(`${file}: ${feature.id} ${action.id} has an invalid fixedD20Result`);
