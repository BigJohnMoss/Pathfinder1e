...
const activeEffectTargets = new Set(["initiative", "armorClass", "fortitude", "reflex", "will", ...abilities, "allies", "self", "area", "enemy"]);
  ...
  for(const replacement of archetype.replacements??[]) {
    ...
    for(const feature of replacement.features??[]) {
      ...
      for (const action of feature.resourceActions ?? []) {
        ...
        if (action.savingThrow && (typeof action.savingThrow.label !== "string" || !action.savingThrow.label.trim() || !abilities.has(action.savingThrow.ability) || !Number.isInteger(action.savingThrow.base) || action.savingThrow.base < 0 || action.savingThrow.base > 30 || !Number.isInteger(action.savingThrow.levelDivisor) || action.savingThrow.levelDivisor < 1 || (action.savingThrow.classId !== undefined && !classIds.has(action.savingThrow.classId)))) errors.push(`${file}: ${feature.id} ${action.id} has an invalid savingThrow`);
        if (action.activeEffect) {
          const effect = action.activeEffect;
          if (typeof effect.name !== "string" || !effect.name.trim() || !Array.isArray(effect.targets) || effect.targets.length === 0 || effect.targets.some(target => !activeEffectTargets.has(target)) || !Number.isInteger(effect.bonus) || effect.bonus < -20 || effect.bonus > 20) errors.push(`${file}: ${feature.id} ${action.id} has an invalid activeEffect`);
