...
const activeEffectTargets = new Set(["initiative", "armorClass", "fortitude", "reflex", "will", "strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma", "allies", "self", "area"]);
    ...
    for(const feature of replacement.features??[]) {
      ...
      for (const action of feature.resourceActions ?? []) {
        ...
        if (action.activeEffect) {
          ...
          if (effect.targets.some(target => target === "self" || target === "area") && (typeof effect.description !== "string" || !effect.description.trim())) errors.push(`${file}: ${feature.id} ${action.id} descriptive activeEffect is missing its description`);
