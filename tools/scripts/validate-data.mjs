...
for (const url of await jsonFiles("archetypes/")) {
  ...
  if (archetype.companionGrants !== undefined) {
    ...
    for (const grant of archetype.companionGrants ?? []) {
      ...
      if (!['animal', 'mount', 'familiar', 'eidolon'].includes(grant?.kind) || typeof grant?.label !== "string" || !grant.label.trim() || typeof grant?.optionId !== "string" || !grant.optionId.trim()) errors.push(`${file}: companion grant ${grant?.id ?? "unknown"} has invalid identity fields`);
